...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (_req, reply) => {
      const client = makePublicClient(
        app.config.chainDefault,
        app.config.alchemyApiKey
      )
      ...
      const blockTimeSkewMs = Math.abs(
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const { chain } = req.query as { chain: ChainKey }
  ...
  app.get(
    '/v1/tokens/:contract/:holder/balance',
    {
      schema: {
        params: z.object({
    ...
    async (req, reply) => {
      const { contract, holder } = req.params as {
        contract: string
        holder: string
      }
      const { chain } = req.query as { chain: ChainKey }
  ...
  app.get(
    ...
    async (req, reply) => {
      const { hash } = req.params as { hash: `0x${string}` }
      const { chain, confirmations } = req.query as {
        chain: ChainKey
      ...
      reply.send({
        ...
        from: tx?.from ?? null,
        to: tx?.to ?? null,
        value: tx?.value ? (tx.value as bigint).toString() : null,
        confirmations: conf,
        finality
      })
    }
